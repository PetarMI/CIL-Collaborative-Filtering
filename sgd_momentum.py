...
def train(k: int, df_train_data: pd.DataFrame, df_test_data: pd.DataFrame):
    ...
    while(i_iter <= paths.sgd_max_iteration):
        ...
        U, M, bu, bi, prev_delta_U, prev_delta_M = sgd_update(train_samples, U, M, alpha, mu, bu,\
            bi, prev_delta_U, prev_delta_M)
...
def sgd_update(train_samples, U, M, l_rate, mu, bu, bi, prev_delta_U, prev_delta_M):
    """ Perform the update step of SGD

    :param train_samples: all samples we are training w.r.t
    :param U, M: Approximation matrices
    :param l_rate: learning rate
    :param prev_delta_U, prev_delta_M: the previous change in U and M - needed for momentum
    :return: updated approximation matrices and biases, along with the change in them 
    """
    beta: float = paths.beta
    l: float = paths.lambda_term

    delta_U = beta * prev_delta_U
    delta_M = beta * prev_delta_M
    ...
    return U, M, bu, bi, delta_U, delta_M 
